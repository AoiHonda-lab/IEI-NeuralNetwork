...
import time
from . import narray_op
from . import preprocessing_layer
from . import output_layer


class IE(nn.Module):
    def __init__(self, dataloader, add=None, tnorm='Algebraic', preprocessing='PreprocessingLayerPercentile'):
        ...
        if self.add > columns_num:
            raise IndexError('"add" must be less than the "number of features"')
        if self.tnorm not in ['Algebraic', 'Min', 'Lukasiewicz', 'Dubois', 'Hamacher']:
            raise ValueError('tnorm / Algebraic, Min, Lukasiewicz, Dubois, Hamacher')
        ...
        #IEILayer
        iei_cls = getattr(narray_op, self.tnorm)
